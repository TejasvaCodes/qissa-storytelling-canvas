...
export function Hero() {
  ...
  return (
    <section id="hero" className="pt-28 md:pt-36">
      <div className="shell grid gap-10 lg:grid-cols-[80px_1fr_320px] lg:gap-14">
        ...
        <div className="order-2 flex gap-3 overflow-x-auto lg:order-1 lg:flex-col lg:overflow-visible">
          {views.map((v, i) => (
            <button
              ...
              onClick={() => setActive(i)}
              aria-label={v.alt}
              className={`w-[68px] shrink-0 transition-opacity duration-500 lg:w-full ${
            ...
            >
              <img
                ...
                width={800}
                height={1008}
                className="h-[86px] w-full object-cover lg:h-[100px]"
              />
            </button>
          ))}
        </div>
          ...
          <div className="relative bg-secondary">
            {views.map((v, i) => (
              <img
                ...
                width={1408}
                height={1760}
                className={`h-[70vh] w-full object-cover transition-opacity duration-[900ms] md:h-[86vh] ${
        ...
        <div className="order-3 lg:pt-6">
          ...
          <p className="mt-8 max-w-[26ch] text-sm leading-relaxed text-muted-foreground">
            A racing silhouette drawn in glove-soft lambskin.
            <br />
            ...
            <div className="mt-5 flex flex-wrap gap-7">
              {sizes.map((s) => (
                <button
                  key={s}
                  onClick={() => setSize(s)}
                  className={`text-sm tracking-[0.14em] transition-colors duration-500 ${
                    size === s
          ...
          <button className="lift eyebrow mt-14 w-full bg-foreground py-5 text-center !text-primary-foreground hover:bg-accent">
            Add to Bag
          </button>
          <button className="eyebrow link-underline mt-8 text-foreground">
            Add to Wishlist
          </button>
        </div>
      </div>
      ...
      {/* sticky add to cart */}
      <div
        className={`fixed inset-x-0 bottom-0 z-40 border-t border-border bg-background/85 backdrop-blur-xl transition-all duration-700 ${
      ...
      >
        <div className="shell flex items-center justify-between gap-6 py-4">
          ...
          <button className="lift eyebrow shrink-0 bg-foreground px-8 py-4 !text-primary-foreground hover:bg-accent">
            Add to Bag
          </button>
        </div>
      </div>
